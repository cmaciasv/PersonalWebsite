...
test.describe('Static Pages Content', () => {
  ...
  test('Home Page should display correct content', async ({ page }) => {
    await page.goto('/');
    await page.waitForTimeout(2200); // Wait for animations to complete
    await runAccessibilityCheck(page);
    await expect(page).toHaveTitle("Carlos Macias Portfolio - Home");
    await expect(page.getByRole('heading', { name: /Hi, I'm Carlos Macias/i })).toBeVisible();
  ...
  test('Contact Page should display correct content and links', async ({ page }) => {
    ...
    await expect(page.getByRole('main').getByText(/Email Me/i)).toBeVisible();
